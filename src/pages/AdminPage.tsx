...
type AdminUser = { last_login_ip?: string | null; last_login_at?: string | null; ip_banned?: boolean; role_ids?: number[]; id: number; name: string; email: string; is_admin: boolean; is_affiliate: boolean; is_streamer: boolean; is_auction: boolean; is_disabled: boolean; created_at: string };
...
export function AdminPage() {
  ...
  const [records, setRecords] = useState<{ipBans?: {ip: string; created_at: string}[]; roles?: { id: number; name: string }[]; userProgramFlagsSupported?: boolean; users: Record<string, unknown>[]; news: AdminNews[]; support: Record<string, unknown>[]; orders: Record<string, unknown>[]; payments: Record<string, unknown>[]; counts: {users: number; orders: number; revenueCentavos: number | null}} | null>(null);
  const [ipSaving, setIpSaving] = useState(false);
  const changeIpBan = async (ip: string, banned: boolean) => {
    if (!window.confirm(banned ? 'Ban ' + ip + '? Everyone sharing this IP will be blocked from the site, including existing sessions.' : 'Unban ' + ip + '?')) return;
    setIpSaving(true); setError('');
    try { await apiRequest('/api/admin/ip-bans', {method: 'PUT', body: JSON.stringify({ip, banned})}); setRecords(await apiRequest('/api/admin')); }
    catch (e) { setError(e instanceof Error ? e.message : 'Unable to change IP ban.'); }
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      {section.id === 'users' && <section className="admin-panel"><h2>Banned IP addresses</h2><p>IP bans affect everyone on the same network. New login IPs are recorded after successful sign-in.</p>{records?.ipBans?.length ? records.ipBans.map(ban => <div key={ban.ip} className="flex items-center justify-between gap-4 py-3"><span>{ban.ip}</span><button type="button" disabled={ipSaving} onClick={() => changeIpBan(ban.ip, false)} className="admin-text-link">Unban IP</button></div>) : <p>No banned IP addresses.</p>}</section>}
      ...
      {section.id === 'orders' && <><DemoOrders admin /><AdminOrders /></>}
